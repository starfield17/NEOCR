...
Schema v2 marks pre-checkpoint non-terminal jobs as `Host.LegacyJobNotResumable`; their `JobSpec` remains for diagnosis. Existing terminal records remain unchanged. The GUI exposes only the current batch job and cancels a paused job on window close. Historical selection and abandoned-job startup reconciliation are separate work.
